...
import com.kingdee.ah.pda.util.AppUtils;
...
//        }
    ...
    public <T> void postByJson( final String url, final Class<T> tClass, JsonObject jsonObject, final VolleyListener<T> listener) {
        com.kingdee.ah.pda.volley.JsonObjectRequest jsonObjectRequest = new com.kingdee.ah.pda.volley.JsonObjectRequest(Method.POST, url, jsonObject, new Listener<JsonObject>() {
            @Override
            public void onResponse(JsonObject response) {
                Logutil.print("网络日志:成功" + response.toString());
                Gson gson = new GsonBuilder().serializeNulls().create();
